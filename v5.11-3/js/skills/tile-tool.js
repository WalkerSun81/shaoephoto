...
var TileToolSkill = {
        ...
        '<div class="tt-panel active" id="ttSplitPanel">' +
            ...
            '<div id="ttIrregularPanel">' +
                ...
                '<div class="tt-section" id="ttRestoreSection">' +
                    ...
                    '<p style="font-size:11px;color:#aaa;margin-bottom:7px">用套索圈出要删除的区域，按"确认删除"将区域内像素变透明</p>' +
                    ...
                    '<div class="tt-btn-group" style="margin-top:5px">' +
                        '<button class="tt-btn tt-btn-primary tt-btn-sm" data-action="applyLassoDelete">确认删除</button>' +
            ...
            '<!-- TRANSFORM MODULE -->' +
            '<div class="tt-section">' +
                '<div class="tt-step-title">移动 / 缩放</div>' +
                '<p style="font-size:11px;color:#aaa;margin-bottom:7px">选中一个精灵后拖拽移动或缩放</p>' +
            ...
            switch (action) {
                ...
                case 'transformMode':
                    ...
                    if (self.state.transformMode === tMode) {
                        ...
                        self._showToast('已退出' + (tMode === 'move' ? '移动' : '缩放') + '模式');
                    } else {
                        ...
                        self._overlayCanvas.style.cursor = tMode === 'move' ? 'grab' : 'nw-resize';
                        self._showToast('已进入' + (tMode === 'move' ? '移动' : '缩放') + '模式，在画布上操作');
                    }
    ...
    _drawIrregularOverlay: function() {
        ...
        regions.forEach(function(region, ri) {
            ...
            var label = '#' + (ri + 1) + ' ' + region.bounds.w + '-' + region.bounds.h + 'px';
    ...
    _onOverlayMouseMove: function(e) {
        ...
        if (this.state.transformDrag && this.state.transformMode === "move" && !this.state._scaleFromCorner && this.state.selectedRegion >= 0) {
            ...
            if (sr4 && this.state.transformStart) {
                ...
                ctx4.fillText(sr4.bounds.w + "-" + sr4.bounds.h + "px", px4, py4 - 16);
            }
            return;
        }
        ...
        if (this.state.transformDrag && this.state._scaleFromCorner && this.state.selectedRegion >= 0) {
            ...
            if (sr5 && this.state.transformInitBounds) {
                ...
                ctx5.fillText(pW5 + "-" + pH5 + "px", ib5.x * ss5, ib5.y * ss5 - 16);
            }
            return;
        }
